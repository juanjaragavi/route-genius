...
4. **Branch discipline** — All work on `dev`. Promotion follows `dev` → `staging` → `main`. Never commit directly to `staging` or `main` (see *Git Workflow & Promotion Hierarchy* below).
